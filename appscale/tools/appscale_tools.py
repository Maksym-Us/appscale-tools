...
class AppScaleTools(object):
  ...
  def show_stats(cls, options):
    ...
    try:
      ...
      print 'host: ' + str(login_host)
      secret = LocalState.get_secret_key(keyname=options.keyname)
      public = LocalState.get_all_public_ips(keyname=options.keyname)
      public = str(json.dumps(public))
      ...
      stats = get_stats(
        ip=public,
        secret=secret,
        nodes=("nodes" in options.show),
        processes=("processes" in options.show),
        proxies=("proxies" in options.show)
      )
      ...
      if nodes_stats is not None:
        ...
        nodes_stats = get_node_stats(nodes_stats=stats["nodes"],
                                     roles=roles_data,
      ...
      if processes_stats is not None:
        ...
        processes_stats = (
          get_proc_stats(proc_stats=proc_stats)
          if not summary
