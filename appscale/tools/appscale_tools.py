...
class AppScaleTools(object):
  ...
  @classmethod
  def render_loadavg(cls, loadavg):
    """
    Returns:
    ...
    return "{} / {} / {}".format(
      last_1 if last_1 < limit_value else cls.get_marked(last_1, "red"),
      last_5 if last_5 < limit_value else cls.get_marked(last_5, "red"),
      last_15 if last_15 < limit_value else cls.get_marked(last_15, "red")
    )


  @classmethod
  def render_partitions(cls, partitions):
    """
    Returns:
    ...
    partitions_info = [
      ...
      if part[1] < 90
      else cls.get_marked("'{}': {}%".format(part[0], part[1]), "red")
  ...
  @classmethod
  def get_roles(cls, roles, ip):
    return roles[ip]


  @classmethod
  def get_node_stats(cls, nodes_stats, specified_roles=None):
    """
    ...
    for ip, node in nodes_stats.iteritems():
      rls = cls.get_roles(roles=roles, ip=ip)
      show = True
      if specified_roles:
      ...
      if show:
        if "shadow" not in rls:
          node_info = [
            ip,
            cls.render_memory(memory=node["memory"]),
            cls.render_loadavg(loadavg=node["loadavg"]),
            cls.render_partitions(partitions=node["partitions_dict"]),
            cls.get_roles(roles=roles, ip=ip)
          ]
        if "shadow" in rls:
          node_info = [
            cls.get_marked(ip, "bold"),
            cls.get_marked(cls.render_memory(memory=node["memory"]), "bold"),
            cls.get_marked(cls.render_loadavg(loadavg=node["loadavg"]), "bold"),
            cls.get_marked(
              cls.render_partitions(partitions=node["partitions_dict"]), "bold"),
            cls.get_marked(cls.get_roles(roles=roles, ip=ip), "bold")
          ]
  ...
  @classmethod
  def sort_proc_stats(cls, proc_stats, column, top=None, reverse=True):
    """
  ...
  @classmethod
  def sort_prox_stats(cls, prox_stats, column):
    """
    Returns sorted input list with reverse is False.
  ...
  @classmethod
  def get_proc_stats(cls, proc_stats, instances=False):
    """
  ...
  @classmethod
  def get_proc_stats_sum(cls, processes_stats, cpu_count):
    """
    ...
    """
    # TODO: get node_stats from HERMES !!!
    cpu_count = sum(int(node["cpu"]["count"])
                    for node in node_stats.itervalues())
  ...
  @classmethod
  def get_prox_stats(cls, proxies_stats, verbose=False, apps_filter=False):
    """
    Obtains useful information from processes summary statistics and returns:
    SERVICE (ID), UNIQUE MEMORY SUM (MB), CPU PER 1 PROCESS (%),
    CPU PER 1 CORE (%), CPU SUM (%) values.
    ...
    for key, value in uniq_proxies.iteritems():
      ...
      prox.append(
        (str(value["hrsp_5xx"])) if not value["hrsp_5xx"]
        else (cls.get_marked(value["hrsp_5xx"], "red"))
             + " / "
             + (str(value["hrsp_4xx"])) if not value["hrsp_4xx"]
        else (cls.get_marked(value["hrsp_4xx"], "red"))
      )
    ...
    return (prox_stats_headers_verbose if verbose else prox_stats_headers), \
           prox_info


  @classmethod
  def get_core_count(cls, node_stats):
    """
  ...
  @classmethod
  def print_table(cls, table_name, headers, data):
    """
    ...
    if data:
      ...
      i = t.find("\n", first_n + 1) - first_n - 1
      AppScaleLogger.log(
        cls.get_marked((("=") * ((i - len(table_name) - 2) / 2))
                       + " " + table_name + " "
  ...
  def show_stats(cls, options):
    ...
    try:
      ...
      if nodes_stats is not None:
        nodes_stats = cls.get_node_stats(nodes_stats=node_stats,
      ...
      if processes_stats is not None:
        ...
        elif "cpu" in options.order_processes or sort is "cpu":
        ...
        processes_stats = (
          ...
          if not summary
          else cls.get_proc_stats_sum(
            processes_stats=proc_stats,
            cpu_count=cls.get_core_count(node_stats=node_stats)
          )
        )
        processes_stats = processes_stats[0], cls.sort_proc_stats(
      ...
      if proxies_stats is not None:
        ...
        proxies_stats = cls.get_prox_stats(
          proxies_stats=prox_stats,
          verbose=verbose,
          apps_filter=apps_only
        )
        proxies_stats = proxies_stats[0], cls.sort_prox_stats(
